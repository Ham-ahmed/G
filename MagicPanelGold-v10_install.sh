...
##setup command=wget -q "--no-check-certificate" https://raw.githubusercontent.com/Ham-ahmed/G/refs/heads/main/MagicPanelGold-v10_install.sh -O - | /bin/sh
...
check_for_updates() {
    ...
    # Try multiple methods to get latest version with improved parsing
    LATEST_VERSION=""
    
    # Try wget first
    LATEST_VERSION=$(wget -q --timeout=20 --tries=3 --no-check-certificate -O - "${GITHUB_BASE}/version.txt" 2>/dev/null | grep -Eo '[0-9]+(\.[0-9]+)*' | head -1)
    
    # If wget failed, try curl
    if [ -z "$LATEST_VERSION" ]; then
        LATEST_VERSION=$(curl -s --connect-timeout 10 --max-time 15 "${GITHUB_BASE}/version.txt" 2>/dev/null | grep -Eo '[0-9]+(\.[0-9]+)*' | head -1)
    fi
    
    # If still empty, try direct download with different user agent
    if [ -z "$LATEST_VERSION" ]; then
        LATEST_VERSION=$(wget -q --timeout=20 --tries=3 --user-agent="Mozilla/5.0" --no-check-certificate -O - "${GITHUB_BASE}/version.txt" 2>/dev/null | tr -d '\r\n\t ' | grep -Eo '[0-9]+(\.[0-9]+)*' | head -1)
    fi
    
    # Validate version format
    if [ -z "$LATEST_VERSION" ] || ! echo "$LATEST_VERSION" | grep -qE '^[0-9]+(\.[0-9]+)*$'; then
        print_message $YELLOW "> Cannot check for updates (invalid version format). Proceeding with installation..."
        return 1
    fi
    
    # Compare versions using custom function
    version_compare "$version" "$LATEST_VERSION"
    local compare_result=$?
    
    if [ $compare_result -eq 2 ]; then
        # New version available (LATEST_VERSION > version)
        echo ""
        print_message $GREEN "####################################################"
        ...
        # Current version is newer (unlikely but possible)
        print_message $GREEN "> You have version $version (latest available: $LATEST_VERSION)"
        return 1
    fi
}
...
# Download the plugin with better error handling
print_message $BLUE "> Downloading..."
DOWNLOAD_URL="${GITHUB_BASE}/MagicPanelGold_v${version}.tar.gz"

# Try multiple download methods
if ! wget -q --no-check-certificate --timeout=30 --tries=5 "$DOWNLOAD_URL" -O "MagicPanelGold_v${version}.tar.gz" 2>/dev/null; then
    print_message $YELLOW "> First download attempt failed, trying alternative method..."
    
    # Try with curl
    if command_exists curl; then
        if ! curl -L --connect-timeout 30 --max-time 60 --insecure -o "MagicPanelGold_v${version}.tar.gz" "$DOWNLOAD_URL" 2>/dev/null; then
            print_message $YELLOW "> Curl download failed, trying with different user agent..."
            wget -q --timeout=30 --tries=3 --user-agent="Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36" --no-check-certificate "$DOWNLOAD_URL" -O "MagicPanelGold_v${version}.tar.gz" 2>/dev/null
        fi
    else
...
if [ -d "MagicPanelGold" ]; then
    ...
    find . -type f \( -name "*.py" -o -name "*.pyo" -o -name "*.pyc" -o -name "*.so" -o -name "*.png" -o -name "*.xml" -o -name "*.json" \) | while read -r file; do
