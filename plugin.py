...
UPDATE_SCRIPT_URL = "https://raw.githubusercontent.com/Ham-ahmed/G/refs/heads/main/MagicPanelGold-v9_install.sh"
...
def check_for_updates(progress_callback=None):
    try:
        ...
        version_url = "https://raw.githubusercontent.com/Ham-ahmed/G/refs/heads/main/version.txt"
        ...
        print(f"الإصدار الحالي: {currversion}, الإصدار الجديد: {latest_version}")
        
        if latest_version > currversion:
...
def check_plugin_changes(progress_callback=None):
    try:
        ...
        plugin_url = "https://raw.githubusercontent.com/Ham-ahmed/G/refs/heads/main/MagicPanelGold.py"
...
class AutoUpdateManager:
    ...
    def check_and_update(self):
        try:
            if self.update_performed:
                print("تم إجراء التحديث مسبقاً، لا تظهر أي نوافذ")
                return
    ...
    def update_check_callback(self, has_update):
        try:
            if has_update:
                ...
                self.last_checked_version = currversion
    ...
    def show_changes_only(self):
        try:
            ...
            self.session.openWithCallback(
                self._load_changes_with_progress,
                MessageBox,
                "جاري تحميل أحدث التغييرات...",
    ...
    def _load_changes_with_progress(self, result=None):
        try:
            progress_screen = self.session.open(UpdateProgress)
            progress_screen.update_progress(0, "جاري تحميل التغييرات...")
            
            progress_timer = eTimer()
            progress_timer.callback.append(lambda: self._load_changes_complete(progress_screen))
            progress_timer.start(1000, True)
                
        except Exception as e:
            print(f"خطأ في بدء تحميل التغييرات: {e}")
            self.session.open(
                MessageBox, 
                f"فشل في تحميل التغييرات.\nالسبب: {str(e)}", 
                MessageBox.TYPE_ERROR
            )

    def _load_changes_complete(self, progress_screen):
        try:
            progress_screen.update_progress(50, "جاري تحليل البيانات...")
            
            changes, new_features = check_plugin_changes()
            
            progress_screen.update_progress(100, "اكتمل التحميل!")
            
            close_timer = eTimer()
            close_timer.callback.append(lambda: self._show_changes_final(changes, new_features, progress_screen))
            close_timer.start(500, True)
                
        except Exception as e:
            print(f"خطأ في عرض التغييرات: {e}")
            progress_screen.close()
            default_changes = [
    ...
    def _show_changes_final(self, changes, new_features, progress_screen):
        progress_screen.close()
...
class ChannelGridMenu(Screen):
        ...
        try:
            ...
            try:
                ...
                if os.path.exists(frame_path):
                 ...
                 # Load MG image
                frame_path = os.path.join(plugin_path, "images", "MG.png")
    ...
    def show_changes_only(self):
        try:
            ...
            self.session.openWithCallback(
                self._load_and_show_changes_only,
                MessageBox,
                "جاري تحميل أحدث التغييرات...",
                MessageBox.TYPE_INFO,
                timeout=2
            ...
            print(f"خطأ في بدء عرض التغييرات: {e}")
            error_details = f"فشل في بدء تحميل التغييرات.\nالسبب: {str(e)}"
            self.session.open(
                MessageBox, 
                error_details, 
                MessageBox.TYPE_ERROR
    ...
    def _load_and_show_changes_only(self, result=None):
        try:
            progress_screen = self.session.open(UpdateProgress)
            progress_screen.update_progress(0, "جاري تحميل التغييرات...")
...
class BasePanel(Screen):
    ...
    def _load_and_show_changes(self, result=None):
        try:
            changes, new_features = check_plugin_changes()
    ...
    def __init__(self, session):
        ...
        self.Novaler_data = {
            ...
            "beengo": {  
                "icon": join(picfold, "nova.png"),
                "channels": [
                    ("beengo-py3.13", "http://ipk.ath.cx/pg/beengo-py3.13.sh", os.path.join(picfold, "nova.png")),  # تم التصحيح هنا
